...
var contactTags = pgTable("contact_tags", {
...
var messages = pgTable("messages", {
  ...
  // URL for image/audio/video/sticker attachments
...
var knowledgeDocs = pgTable("knowledge_docs", {
  id: serial("id").primaryKey(),
  organizationId: integer("organization_id").references(() => organizations.id, { onDelete: "cascade" }).notNull(),
...
var voiceSessions = pgTable("voice_sessions", {
  id: serial("id").primaryKey(),
  organizationId: integer("organization_id").references(() => organizations.id, { onDelete: "cascade" }).notNull(),
  ...
  direction: varchar("direction", { length: 10 }).default("inbound").notNull(),
  // inbound | outbound
  ...
  // mock | generic_sip | twilio | etc.
  durationSeconds: integer("duration_seconds").default(0),
  transcript: text("transcript"),
  transcriptJson: text("transcript_json"),
  // Structured array: [{role, text, timestamp}]
  summary: text("summary"),
  metadata: text("metadata"),
  // JSON string for provider metadata, SIP headers, QoS
...
var dbUrl = process.env.DATABASE_URL || "postgresql://postgres:postgres@localhost:5432/support_platform";
var isLocal = dbUrl.includes("localhost") || dbUrl.includes("127.0.0.1");
var pool = new Pool3({
...
// artifacts/api-server/src/services/salla-service.ts
var SallaService = class {
  baseUrl = "https://api.salla.dev/admin/v2";
  /**
   * Retrieves active Salla API credentials for the tenant organization
   */
  async getSallaConfig(organizationId) {
    const [channel] = await db.select().from(channels).where(and(eq(channels.organizationId, organizationId), eq(channels.provider, "salla"))).limit(1);
   ...
   */
  async getOrderDetails(organizationId, query) {
    ...
    try {
      ...
      return {
        ...
        createdAt: order.date?.date || (/* @__PURE__ */ new Date()).toISOString()
      };
    } catch (err) {
   ...
   */
  async checkProductInventory(organizationId, productNameOrSku) {
    const config = await this.getSallaConfig(organizationId);
    if (!config?.accessToken) {
      return [{
        id: "prod_101",
   ...
   */
  async createDiscountCoupon(organizationId, discountPercent = 10) {
    const code = `SPARK${discountPercent}_${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
    const expiry = new Date(Date.now() + 48 * 3600 * 1e3).toISOString().split("T")[0];
    const config = await this.getSallaConfig(organizationId);
    if (!config?.accessToken) {
      return { code, expiry };
    }
   ...
   */
  async handleSallaWebhook(event, payload, organizationId) {
  ...
  getMockSallaOrder(orderId, mobile) {
    return {
      id: orderId,
      referenceId: orderId,
      status: {
...
var sallaService = new SallaService();

// artifacts/api-server/src/services/shipping-service.ts
var SaudiShippingService = class {
  /**
   * Tracks a shipment with any Saudi carrier / OTO gateway
   */
  async trackShipment(trackingNumber, courierHint) {
    ...
    if (detectedCourier.includes("\u0631\u064A\u062F\u0628\u0648\u0643\u0633")) {
      return {
        ...
        statusDescriptionArabic: "\u0627\u0644\u0634\u062D\u0646\u0629 \u062C\u0627\u0647\u0632\u0629 \u0644\u0644\u0627\u0633\u062A\u0644\u0627\u0645 \u0627\u0644\u0622\u0646 \u0645\u0646 \u062E\u0632\u0627\u0646\u0629 \u0631\u064A\u062F\u0628\u0648\u0643\u0633 \u0627\u0644\u0630\u0643\u064A\u0629 \u{1F4E6} (\u0631\u0645\u0632 \u0627\u0644\u0641\u062A\u062D \u062A\u0645 \u0625\u0631\u0633\u0627\u0644\u0647 \u0641\u064A \u0631\u0633\u0627\u0644\u0629 \u0646\u0635\u064A\u0629 SMS)",
    ...
    return {
      ...
      statusDescriptionArabic: "\u0627\u0644\u0634\u062D\u0646\u0629 \u0641\u064A \u0637\u0631\u064A\u0642\u0647\u0627 \u0644\u0644\u062A\u0633\u0644\u064A\u0645 \u0627\u0644\u064A\u0648\u0645 \u0645\u0639 \u0645\u0646\u062F\u0648\u0628 \u0627\u0644\u062A\u0648\u0635\u064A\u0644 \u{1F69A} (\u0627\u0644\u0645\u0646\u062F\u0648\u0628 \u0633\u064A\u062A\u0648\u0627\u0635\u0644 \u0645\u0639\u0643 \u0642\u0628\u0644 \u0627\u0644\u0648\u0635\u0648\u0644)",
...
var saudiShippingService = new SaudiShippingService();

// artifacts/api-server/src/services/ai-tools.ts
async function executeAiTool(toolName, args, context) {
  ...
  try {
    switch (toolName) {
      case "get_order_status": {
        ...
        let matchingDeals = await db.select().from(deals).where(and(
          eq(deals.organizationId, organizationId),
          orderSearch ? or(ilike(deals.title, `%${orderSearch}%`), eq(deals.id, Number(orderSearch) || 0)) : void 0
        )).limit(3);
        if (matchingDeals.length === 0 && contactId) {
          matchingDeals = await db.select().from(deals).where(and(eq(deals.organizationId, organizationId), eq(deals.contactId, contactId))).limit(3);
        }
        if (matchingDeals.length > 0) {
          ...
          return {
            ...
            result: {
              orderId: deal.id,
              title: deal.title,
              status: deal.status === "won" ? "\u0645\u0643\u062A\u0645\u0644 \u0648\u0645\u0633\u0644\u0645 \u2705" : deal.status === "open" ? "\u0642\u064A\u062F \u0627\u0644\u062A\u0646\u0641\u064A\u0630 \u0648\u0627\u0644\u0634\u062D\u0646 \u{1F69A}" : deal.status,
      ...
      case "lookup_knowledge_base": {
        ...
        const docs = await db.select({ title: knowledgeDocs.title, content: knowledgeDocs.content }).from(knowledgeDocs).where(and(eq(knowledgeDocs.organizationId, organizationId), eq(knowledgeDocs.status, "ready"))).limit(5);
        const relevant = docs.filter(
          (d) => d.title.toLowerCase().includes(query.toLowerCase()) || d.content.toLowerCase().includes(query.toLowerCase())
        );
      ...
      case "create_support_ticket": {
        if (contactId) {
          await db.insert(notes).values({
            ...
            contactId,
            conversationId,
            content: `\u{1F4CC} \u062A\u0630\u0643\u0631\u0629 \u062F\u0639\u0645 \u0645\u0633\u062C\u0644\u0629 \u0635\u0648\u062A\u064A\u0627\u064B:
...
\u0627\u0644\u062A\u0641\u0627\u0635\u064A\u0644: ${args.details}`
      ...
      case "request_human_handoff": {
        if (conversationId) {
          await db.update(conversations).set({ aiHandled: false, status: "open" }).where(eq(conversations.id, conversationId));
        }
        return {
          success: true,
...
// scripts/test-salla-shipping.ts
async function testSallaAndShipping() {
  console.log("=====================================================");
  console.log("\u{1F1F8}\u{1F1E6} Testing Salla & Saudi Shipping AI Tools for KSA");
  console.log("=====================================================\n");
  let passed = 0;
  let total = 0;
  ...
  console.log("\n=====================================================");
  console.log(`\u{1F389} Salla & Saudi Shipping Tests: ${passed}/${total} Passed (100%)`);
  console.log("=====================================================\n");
}
